...
$s1 = $conn->prepare("SELECT id AS order_id, shipment_status FROM cart_orders WHERE user_id = ? AND status = 'paid'");
if ($s1) { $s1->bind_param("i",$uid); $s1->execute(); $result['cart'] = $s1->get_result()->fetch_all(MYSQLI_ASSOC); }

$s2 = $conn->prepare("SELECT auction_id, status AS shipment_status FROM auction_claims WHERE user_id = ? AND choice='delivery'");
